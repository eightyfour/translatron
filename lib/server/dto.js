    ...
    return {
        ...
        createNewProject : function (id, path, projectName, obj, cb) {
            ...
            getDefaultProjectJson(projectName, obj, (json) => {
                ...
                jsonFileManager.saveJSON(nPath, projectName + '.json', json);
                cb(json);
            });
        },
        /**
        ...
        sendResource : function (id, bundle, data, cb) {
            jsonFileManager.getJSON(bundle.projectId + '.json', (projectObj) => {
                ...
                jsonFileManager.saveJSON(url === '' ? '/' : url, bundle.project + '.json', projectObj , (err) => {
                    cb(data.key);
        ...
        renameKey : function (id, bundleObj, data, cb) {
            ...
            if (bundleObj.locales) {
                ...
                bundleObj.locales.forEach(function (lang) {
                    keyValueFileManager.renameKey(generatePathName({bundle : bundleObj.bundle, locale: lang}), data.newKey, data.oldKey, function (newKey, oldKey, value) {
                        l--;
                        if (l <= 0) {
