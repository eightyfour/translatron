    ...
    api = {
      ...
      saveKey: function(projectId, language, keyAndValue, cb) {
        dao.saveKey(
          ...
          language,
          keyAndValue,
          (err, keyName, keyValue) => {
            if (!err) {
              cb(null, keyName, keyValue)
              changesNotifier.notify(
                "keyUpdated",
                [projectId, language, keyName, keyValue],
                clientListenerId
              )
            } else {
              cb(err)
      ...
      /**
       * @deprecated use copyKey instead
       * @param {*} projectId
       * @param {*} keyAndValue
       * @param {*} cb
       */
      ...
      copyKey: function(projectId, { key, newKey }, cb) {
        dao.copyKey(projectId, { key, newKey }, (err, projectId, data) => {
          if (!err) {
            cb(null, projectId, data)
            changesNotifier.notify(
              "onKeyCopied",
      ...
      renameKey: function(projectId, renameData, cb) {
        dao.renameKey(projectId, renameData, (err, oldKeyName, newKeyName) => {
          if (!err) {
            ...
            changesNotifier.notify(
              "keyRenamed",
              [projectId, oldKeyName, newKeyName],
              clientListenerId
            )
          } else {
            cb(err)
      ...
      createNewProject: function(path, projectName, obj, cb) {
        dao.createNewProject(
          path,
          projectName,
          obj,
          (err, projectData, project) => {
            if (!err) {
