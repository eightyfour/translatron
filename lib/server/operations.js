...
module.exports = function(dao, changesNotifier, authConfig) {
    ...
    var clientListenerId = Math.trunc(Math.random() * Math.pow(10, 10)),
        ...
        api = {
            ...
            deleteProject: function(currentDirId, projectName, cb) {
                if (!hasAdminRights) {
                    cb(new Error('Not allowed to perform this action.'));
                    return;
                }

                dao.deleteProject(currentDirId, projectName, (err) => {
                    if (!err) {
                        cb(null, projectName);
                        changesNotifier.notify('projectDeleted', [projectName], clientListenerId);
                    } else {
                        cb(err);
                    }
                });
            },
            deleteFolder: function(currentDirId, dirName, cb) {
                if (!hasAdminRights) {
                    cb(new Error('Not allowed to perform this action.'));
                    return;
                }

                dao.deleteFolder(currentDirId, dirName, (err) => {
                    if (!err) {
                        cb(null, dirName);
                        changesNotifier.notify('folderDeleted', [dirName], clientListenerId);
                    } else {
                        cb(err);
                    }
                });
