                    ...
                    files.forEach(function (file) {
                        fs.lstat(path + '/' + file, function (err, stats) {
                            if (!err) { //conditing for identifying folders
                                res.push({d: stats.isDirectory(), name: file, id: path + '/' + file});
                            } else {
        ...
        fs.readdir(path, function (err, files) { // '/' denotes the root folder
            if (err) {
                ...
                files.forEach(function (file) {
                    fs.lstat(path + '/' + file, function (err, stats) {
                        if (!err) { //conditing for identifying folders
                            res.push({d: stats.isDirectory(), name: file, id: dir + '/' + file});
                        } else {
                            console.log('fileManager:readDir :: ERROR');
                        }
                        count--;
...
//            cb(obj, code);
        ...
        readDir : function (obj, cb) {
            readDir(isNaN(obj[0]) ? path.normalize(projectFolder + '/' + obj) : idToPath(obj), cb);
        },
        listDir : function (dir, cb) {
            listDir(dir, path.normalize(projectFolder + '/' + dir), cb);
        },
