...
var dao = function (storageDirectory) {
    ...
    return {
         ...
         * @param cb
         */
        createNewProject : function (id, parentDirectory, projectName, obj, cb) {
            ...
            var projectPath = path.normalize(storageDirectory + '/' + parentDirectory + '/') + projectName + '.json';
        ...
        createNewDirectory : function(id, directoryName, parentDirectory, cb) {
            // QUESTION: is this a good node pattern? (esp. in connection with callback?)
        ...
        saveKey : function (id, projectId, language, keyAndValue, cb) {
            this.loadProject(projectId, (project) => {
                ...
                fs.writeFile(storageDirectory + projectId + '.json', JSON.stringify(project, null, 2), (err) => {
                    if (err) {
                        cb(false);
                    } else {
