...
var dao = function({projectFolder, uploadFolder, projectJSON}) {
    ...
    return {
        ...
        createNewProject: function(parentDirectory, projectName, obj, cb) {
            ...
            projectHandler.read()
                .then((projects) => {
                    if (projects[parentDirectory + projectName])
                        throw 'Project already exists'
                    
                    const newProject = {
                        name : projectName,
                        url : projectData.projectId,
                        file : projectData.projectId + '.json'
                    }
                    
                    projects[parentDirectory + projectName] = newProject
                    
                    return projectHandler.save({projects}).then(() => {
                ...
                .then(({projectData, newProject}) => cb(null, projectData, newProject))
                .catch(err =>  cb(err))
        },
        /**
         *
        ...
        deleteProject: function(parentDirectory, projectName, cb) {
            ...
            const id = parentDirectory + projectName
            ...
            projectHandler.read()
                .then(projects => {
                    ...
                    return new Promise((resolve, reject) => {
                        fs.unlink(projects.projects[id].file, function(err) {
                            if (err)
                                resolve(err)
                            else
                                resolve()
