 ...
 */
var dao = function(storageDirectory, uploadDirectory) {
    "use strict";
    ...
    return {
        ...
        cloneKey: function(projectId, keyAndValue, cb) {
            this.loadProject(projectId, (projectData) => {
                ...
                var translations = projectData.keys,
                    ...
                    currentLang,
                    result = {
                        key: [keyAndValue.targetCategory, keyAndValue.key].join('_'),
                        values: {}
                    };
                ...
                for (var i = 0; i < keys.length; i++) {
                    ...
                    if (currentBundle.hasOwnProperty(keyAndValue.id)) {
                        currentBundle[keyAndValue.targetCategory + '_' + keyAndValue.key] = currentBundle[keyAndValue.id];
        ...
        importJSON: function(projectId, data, cb) {
            this.loadProject(projectId, (projectData) => {
                ...
                for (var i = 0; i < uploadedLanguages.length; i++) {
                    ...
                    currentKeys = localKeys[currentLanguage] || {};

                    if (availableLanguages.indexOf(currentLanguage) > -1) {
                        uploadedKeys = data[currentLanguage];

                        for (var key in uploadedKeys) {
                            if (uploadedKeys.hasOwnProperty(key)) {

                                // Check on valid json keys
                                if (key.indexOf('_') === -1) {
                                    if (typeof uploadedKeys[key] === 'string') {
                                        // Not supported as key should at least have a category
                                        cb(new TypeError(`Key '${key}' not compatible with Translatron project, has to provide both category and key id.`));
                                        return;
                                    } else if (typeof uploadedKeys[key] === 'object') {
                                        // If keys are provided by category objects store them as Translatron internally used format
                                        for (var childKey in uploadedKeys[key]) {
                                            if (uploadedKeys[key].hasOwnProperty(childKey)) {
                                                currentKeys[`${key}_${childKey}`] = uploadedKeys[key][childKey];
                                            }
                                        }
                                    }
                                } else {
                                    currentKeys[key] = uploadedKeys[key].toString();
                                }
                            }
                        }

                        if (!localKeys[currentLanguage]) {
                            localKeys[currentLanguage] = currentKeys;
                        }
                    }
                }
        ...
        removeImage: function(projectId, categoryName, callback) {
            this.loadProject(projectId, (projectData) => {
                ...
                fullPath = path.normalize(uploadDirectory + '/' + projectData.projectId + '/' + projectData.images[categoryName]);

                if (!fs.existsSync(fullPath)) {
                    sendError(`Could not delete image. File ${fullPath} does not exist.`);
                }
                else {
