var fileManagerObj = require('./legacy/fileManager'),
    fs = require('fs'),
    ProjectHandler = require('./lib/ProjectHandler'),
    path = require('path');
...
var dao = function({projectFolder, uploadFolder, projectJSON}) {
    ...
    /**
     * Persist a project as a file below "projectFolder". "projectData" should be complete and well-formed (especially
     * the projectId).
    ...
    function writeProjectConfigFile(projectData, callback) {
        ...
        var projectFilePath = projectFolder + '/' + projectData.projectId + '.json';
    ...
    return {
        ...
        loadProject: function(projectId, cb) {
            ...
            projectHandler.read().then(projects => {
               // console.log('dao:', projects);
            }).catch(err => console.log(err))
            
            var fullPath = path.normalize(projectFolder + '/' +  projectId + '.json');
         ...
         * @param cb
         */
        deleteProject: function(parentDirectory, projectName, cb) {
            if (parentDirectory[parentDirectory.length - 1] !== '/') {
                parentDirectory += '/';
            }

            var pathToFile = projectFolder + parentDirectory + projectName + '.json';
         ...
         * @param cb
         */
        deleteFolder: function(parentDirectory, dirName, cb) {
            if (parentDirectory[parentDirectory.length - 1] !== '/') {
                parentDirectory += '/';
            }

            var folderPath = projectFolder + parentDirectory + dirName;
        ...
        getDirectory: function(dir, cb) {
            if (dir[0] === '/') {
                ...
                projectHandler.listFiles(dir).then(projectList => {
                    console.log('dao:getDirectory', projectList)
