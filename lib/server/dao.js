...
var dao = function (dirName) {
    ...
    function computeIdFromPath(path) {
        return path.replace(new RegExp(dirName), "");
    }
    ...
    return {
        ...
        loadProject : function (projectId, cb) {
            jsonFileManager.getJSON(projectId, (data) => {
                if (data) {
                    cb(data);
                } else {
        ...
        getDirectory : function(dir, cb) {
            fileManager.readDir(dir, function(entry) {
                var directories = [],
                    projects = [];
                if (entry) {
                    ...
                    cb({projects : projects, dirs : directories, parentDirectory: fileManager.getParentDirectory(dir)});
                } else {
                    cb(false);
                }
            });
