...
var projectConfigTemplate = {
   ...
   */
  availableLanguages: ["en", "de", "fr", "es", "nl", "da", "sv", "en_GB"],
...
function createInitialProjectConfig(projectName, obj) {
  ...
  projectObj.description =
    (obj && obj.description) || projectConfigTemplate.description
  return projectObj
}
    ...
    switch (typeof value) {
      ...
      case "string":
        return new TypeError(
          `Key '${key}' not compatible with Translatron project, has to provide both category and key id.`
        )
        break
...
var dao = function({ projectFolder, uploadFolder, projectJSON }) {
  ...
  if (!fs.statSync(projectFolder).isDirectory()) {
    throw new Error(
      "dao.projectFolder " + projectFolder + " is not a directory"
    )
  }
  ...
  return {
    ...
    loadProject: function(projectId, cb) {
      ...
      projectHandler
        .read()
        .then(projects => {
          if (!projects.projects[projectId]) throw projectId + " does not exist"
        ...
        .then(({ name, url, file }) => {
          return new Promise((resolve, reject) => {
            fs.readFile(
              path.normalize(projectFolder + "/" + file),
              "utf8",
              (err, dataString) => {
                if (err) reject(err)
                else resolve({ id: projectId, name, url, file, dataString })
              }
            )
          })
        })
        ...
        .then(({ id, name, url, file, data }) => {
          !data.availableLanguages &&
            (data.availableLanguages = projectConfigTemplate.languages)
          ...
          return projectHandler.save(projectsJSON).then(() => {
            ...
            return new Promise(resolve => {
              writeProjectConfigFile(
                ...
                err => {
                  if (!err) resolve({ projectData, newProject })
                  else throw err
                }
              )
            })
    ...
    createNewDirectory: function(directoryName, parentDirectory, cb) {
      var fullParentDirPath = path.normalize(
        projectFolder + "/" + parentDirectory
      )
      ...
      if (!fs.existsSync(fullParentDirPath)) {
        console.error(
          "dao.createNewDirectory: cannot create directory " +
            fullPath +
      ...
      fs.mkdir(fullPath, function(exception) {
        if (exception) {
          console.error(
            "dao.createNewDirectory: error creating directory " +
              fullPath +
              ": " +
              exception
          )
      ...
      this.loadProject(projectId, (projectData, { id, name, url, file }) => {
        if (!projectData) {
          var msg = [
            "Project",
            projectId,
            "does not exist, cannot update decription"
          ].join(" ")
        ...
        writeProjectConfigFile(
          { data: projectData, id, name, url, file },
          err => {
            if (!err) {
              callback(null)
            } else {
    ...
    renameCategory: function(projectId, oldName, newName, cb) {
      this.loadProject(projectId, (projectObj, { id, name, url, file }) => {
        if (projectObj.keys) {
          ...
          writeProjectConfigFile(
            { data: projectObj, id, name, url, file },
            err => {
              if (!err) {
    ...
    deleteProject: function(projectId, cb) {
      ...
      projectHandler
        .read()
        .then(projectJSON => {
          if (!projectJSON.projects[id]) throw "Project does not exists " + id

          return new Promise((resolve, reject) => {
            fs.unlink(
              path.normalize(projectFolder + projectJSON.projects[id].file),
              function(err) {
                if (err) reject(err)
                else resolve()
              }
    ...
    copyKey: function(projectId, { key, newKey }, cb) {
      this.loadProject(projectId, (projectData, { id, name, url, file }) => {
        if (!projectData) {
          var msg = [
            "Project",
        ...
        const translations = projectData.keys
        ...
        for (var i = 0; i < keys.length; i++) {
          let lang = keys[i]
          let currentBundle = translations[lang]
          if (currentBundle.hasOwnProperty(key)) {
            result.keys[lang] = {}
            currentBundle[newKey] = currentBundle[key]
            result.keys[lang][newKey] = currentBundle[newKey]
          }
        }

        cloneKeyDescription(projectData.keyDescriptions, key, newKey)
        if (projectData.keyDescriptions[newKey]) {
          result.keyDescriptions[newKey] = projectData.keyDescriptions[newKey]
        }
      ...
      projectHandler
        ...
        .then(projects => {
          if (!projects.projects[id])
            throw {
              error: ERRORS.PROJECT_NOT_EXISTS,
          ...
          if (url && url[0] !== "/")
            throw {
              ...
              message: "invalid url - it must start with a slash!"
            }

          return projects
        })
        .then(projects => {
          if (projects.projects[id].url === url) return projects
          ...
          if (availableLanguages.indexOf(currentLanguage) > -1) {
            ...
            for (var key in uploadedLangKeys) {
              if (uploadedLangKeys.hasOwnProperty(key)) {
                currentKey = getTranslationsForKey({
                  key: key,
                  uploadedKeys: uploadedLangKeys
                })
    ...
    getDirectory: function(dir, cb) {
      if (dir[0] === "/") {
        ...
        console.error(
          "dao:ask for a not valid directory type - all directories must be start with a slash!"
        )
        cb(false)
      }
    },
      ...
      this.loadProject(projectId, (projectData, { id, name, url, file }) => {
        if (!projectData) {
          var msg = [
            "Project",
            projectId,
            "does not exist, cannot add the new image"
          ].join(" ")
          console.warn(msg)
          callback(new Error(msg))
          return
    ...
    removeImage: function(projectId, categoryName, callback) {
      this.loadProject(projectId, (projectData, { id, name, url, file }) => {
        ...
        if (!projectData) {
          sendError(
            `Project ${projectId} does not exist, cannot remove image for category ${categoryName}`
          )
          return
        } else if (!projectData.images || !projectData.images[categoryName]) {
          sendError(
            `Value for key category ${categoryName} not found in project configuration.`
          )
          return
        }
        ...
        if (projectData.images[categoryName][0] === "/") {
          fullPath = path.normalize(
            uploadFolder + "/" + projectData.images[categoryName]
          )
        } else {
          fullPath = path.normalize(
            uploadFolder +
              ...
              "/" +
              projectData.images[categoryName]
          )
        }
        ...
        if (!fs.existsSync(fullPath)) {
          sendError(
            `Could not delete image. File ${"/" +
              projectData.projectId +
              "/" +
              projectData.images[categoryName]} does not exist.`
          )
          return
        } else {
