var changesNotifier = function() {
    ...
    return {
         ...
         * @param eventPayload payload for the event as an Array (each element will become a parameter to the client side
         * callback function, in order of array)
        ...
        notify: function (event, eventPayload, eventSourceListenerId) {
            Object.keys(listeners).forEach( (listenerId) => {
                ...
                if (listener.hasOwnProperty(event)) {
                    ...
                    listener[event].apply(null, eventPayload);
                };
            });
        }
    };
