...
var projectOverviewController = (function() {
    ...
    projectOverview.onToggleEditMode(function() {
        // TODO: Show trashcans beneath project links
    });

    projectOverview.onDeleteProjectPressed(function(projectName) {
        trade.deleteProject(projectName, currentDirectory, function(name) {
            var toastMessage;
            if (name) {
                projectOverview.deleteProjectNode(name);
                toastMessage = 'Project "' + name + '" has been deleted.';
            } else {
                toastMessage = 'There was an error deleting project "' + name + '".';
            }
