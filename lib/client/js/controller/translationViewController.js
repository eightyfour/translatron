...
translationView.setupEvents({
    ...
    sendResource : function (key, lang, value) {
        ...
        trade.sendResource({
            ...
            locale: lang || projectConfig.defaultLanguage
        }, {
            key: key,
            value: value || undefined
        }, function (project, lang, key, value) {
            if (project === projectConfig.project) {
                collectKeysMap[key] = undefined; // save the key
                translationView.printBundleTemplate([{key:key, value: value || ''}], lang, availableLanguages, project);
...
function handleNewProjectConfig (newProjectConfig) {
    if (!newProjectConfig.hasOwnProperty('projects')) {
        ...
        trade.loadProject(newProjectConfig.projectId, function (error) {
            // callback is only called if an error occurs
            console.error('translationViewController:loadProject fails for projectId:', newProjectConfig.projectId);
        });
