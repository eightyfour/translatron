...
var translationView = (function() {
    ...
    // methods needs to be implemented on controller side
        ...
        fc = {
                ...
                var keyObj,
                    ...
                    prepareCategoryNode = function(node, keyObj, languages) {
                        ...
                        if (!categoryNode) {
                            ...
                            var overallWordCount = categoryNode.querySelector('.overallWordCountWrapper .translationContainer');
                            languages.forEach(function(lang) {
                                var countWrapper = domOpts.createElement('div', null, 'data js_overall_' + lang),
                                    wordCount = domOpts.createElement('span', null, 'wordCountLabel');
                                wordCount.innerHTML = getWordCountText(4);
                                countWrapper.appendChild(wordCount);
                                overallWordCount.appendChild(countWrapper);
                            });
                        }
                        return categoryNode;
                    };
            ...
            /**
             * Update the wordCount for a given category
             * @param data
             */
            updateCategoryWordCount: function(data) {
                var label = document.querySelector('#' + data.id + ' .js_overall_' + data.language + ' .wordCountLabel');
                if (label) {
                    label.innerHTML = getWordCountText(data.wordCount);
                }
            },
            /**
             * creates a key field
            ...
            addRowWithLanguages: function(node, data, actualLanguage, allProjectLanguages) {
                ...
                allProjectLanguages.forEach(function(lang) {
                    fc.addLanguageField(row.querySelector('.translationContainer'), data.key, actualLanguage === lang ? data.value : null, lang, data.words || 0);
                });
            },
