    ...
    Object.keys(conf).forEach(function (prop) {
        ...
        if (prop === 'inputTransPrefix') {
            ...
            if (node) {
                ...
                console.error('translationView:renameIds can not find dom node for id', oldKey + conf[prop]);
            }
        }
    });
}
...
var translationView = (function () {
    ...
    function SaveOnLeave(node, key, lang, text) {
        ...
        function getIdFromRow(node) {
            var id = node.parentNode.parentNode.getAttribute('id').replace(conf.rowPrefix,'');
