...
var projectMainNavigation = (function () {
    ...
    var mainNode,
        ...
        modViews = {
            ...
            showMessageBundleFormat : function (node) {
                node.addEventListener('click', function () {
                    alert('TODO - Show a list of all available project links dependence on existing languages.\n\nEach link will open the message bundle raw data in a new tab.')
                });
            },
            ...
            showJSONBundle : function(node) {
                node.addEventListener('click', function() {
                    alert('TODO - open new tab an show the JSON raw data for this project');
                });
            },
            from : function (node) {
                this.from.node = node;
    ...
    function setProjects(projects, node) {
        ...
        projects.forEach(function (prjItem) {
            var span = window.domOpts.createElement('span');
            li = window.domOpts.createElement('li');
            li.addEventListener('click', function () {
                uiEvents.callUievent('projectSelected', prjItem.id);
            });
            span.innerHTML = prjItem.name;
            span.domAppendTo(li);
            li.domAppendTo(ul);
