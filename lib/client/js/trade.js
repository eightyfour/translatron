    ...
    return {
        ...
        deleteProject : function (projectName, currentDirId, callback) {
            if (isConnected()) {
                ...
                if (currentDirId[currentDirId.length - 1] !== '/') {
                    currentDirId += '/'
                }

                server.deleteProject(currentDirId + projectName,
                    function (err, prjName) {
                        if (!err) {
                            // TODO change to: prjName
                            cb && cb(null, projectName)
                            callController('projectDeleted', [projectName])
                        } else {
                            cb && cb(err)
                        }
                    })
        ...
        deleteFolder : function (dirName, currentDirId, callback) {
            if (isConnected())
                server.deleteFolder(currentDirId, dirName,
