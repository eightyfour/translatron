...
var trade = (function() {
  ...
  function init(s, fc) {
    ...
    if (server.setUserRights) {
      server.setUserRights(
        canny.cookieManager.forSessionCookie("translatron_session").getValues(),
        fc
      );
    } else {
      fc(
  ...
  function createConnection(fc) {
    ...
    d.on("end", function(err) {
      ...
      if (tryToReconnect) connectionLostUi.render("RECONNECT_FAIL");
      else setTimeout(() => connectionLostUi.render("CONNECTION_END"), 1000);
    });
  ...
  return {
    ...
    createKey: function(projectId, language, keyAndValue, cb) {
      if (isConnected())
        server.saveKey(projectId, language, keyAndValue, function(
          err,
          key,
          value
        ) {
    ...
    cloneKey: function(projectId, keyAndValue, cb) {
      if (isConnected())
        server.cloneKey(projectId, keyAndValue, function(err, projectId, data) {
    ...
    createNewProject: function(projectName, currentDirId) {
      if (isConnected())
        server.createNewProject(currentDirId, projectName, {}, function(
          err,
          projectData,
          project
        ) {
    ...
    renameCategory: function(projectId, oldName, newName) {
      if (isConnected())
        server.renameCategory(projectId, oldName, newName, function(
          err,
          oldName,
          newName
        ) {
    ...
    removeImage: function(projectId, categoryName) {
      if (isConnected())
        server.removeImage(projectId, categoryName, function(
          err,
          categoryName
        ) {
          if (!err) {
    ...
    getDirectory: function(dir, cb) {
      if (isConnected())
        server.getDirectory(dir, function(err, args) {
          if (err !== null) {
            toast.showMessage(
              "Internal server error! Please report this message to a developer: " +
                err.message
            );
    ...
    saveProjectDescription: function(projectId, id, description, callback) {
      if (isConnected())
        server.saveProjectDescription(projectId, id, description, function(
          err
        ) {
          if (!err) {
    ...
    deleteFolder: function(id, callback) {
      if (isConnected())
        server.deleteFolder(id, function(err, dirId) {
          if (!err) {
