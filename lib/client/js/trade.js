...
var trade = (function () {
    ...
    var cbs = [],
        ...
        d = dnode(); // continue here to create the connection again if lost - then check if possible (because auth and so on...)
    
    /**
     * Check if conection is available - otherwise send view error
     * @returns {boolean}
     */
    function isConnected() {
        ...
        alert('Connection lost - please relaod the page')
        return false
    }
    ...
    return {
        initialize : function (fc) {
            ...
            d.on('fail', function (server) {
                hasServerConnection = false
                console.error('trade:fail', arguments);
                alert('Connection fail - please refresh the page')
            })
            d.on('error', function (server) {
                hasServerConnection = false
                console.error('trade:error', arguments);
            ...
            d.on('end', function (server) {
                ...
                alert('Connection end - please refresh the page')
            })
