...
var trade = (function () {
    ...
    return {
        ...
        getPropertiesFiles : function(bundle, cb) {
            ...
            var filesCompleteCount = 0;
            var dataObj = {};

            server.getPropertiesFiles(bundle, function(files) {
                files.forEach(function(file) {
                    server.fileManager.getFile(file.id, function(fileObj) {
                        filesCompleteCount++;
                        dataObj[getLanguage(file.name)] = fileObj;
                        if (filesComplete()) {
                            cb(dataObj);
                        }
                    })
                ...
                function getLanguage(fileName) {
                    var reg = new RegExp('messages_(.*)\.properties');
                    return reg.exec(fileName)[1];
                }

                function filesComplete() {
                    return filesCompleteCount === files.length;
                }
            });
        },
        /**
